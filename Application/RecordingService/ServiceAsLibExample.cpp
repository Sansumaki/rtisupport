...
void process_role(
	rti::recording::ApplicationRoleKind role,
	std::string config,
...
)
{
	...
	service_property.application_role(role);
	service_property.cfg_file(config);
	// Check if a role argument has been provided
	/*if (argc > 1) {
		if (recorder_role.compare(n_argv[1]) == 0) {
			service_property.application_role(role);
			service_property.cfg_file("recorder_config.xml");
		}
		else if (replay_role.compare(n_argv[1]) == 0) {
			service_property.application_role(role);
   ...
   /* if (argc > 2) {
		std::stringstream domain_id_stream(n_argv[2]);
		int32_t domain_id = 0;
	...
	//if (argc > 3) {
	//    std::stringstream running_secs_stream(n_argv[3]);
...
int mainx(rti::recording::ApplicationRoleKind role, std::string config, std::int32_t domain_id)
{
	...
	//process_role(argc, n_argv, service_property, running_seconds);
/*}
catch (const std::exception & ex) {
	std::cerr << "Exception: " << ex.what() << std::endl;
	print_usage(n_argv[0]);
	return EXIT_FAILURE;
}*/
